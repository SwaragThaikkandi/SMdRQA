# Import packages needs formatting
import numpy as np
...
# replace this part with the directory where you have saved the
# datafiles(only the data files)
dir = '/home/swarag0/Group_study/Polar-GrpStudyHR-Data'
files = [f for f in listdir(dir) if isfile(join(dir, f))]
# Total number of data files in our experiment was 489, this line of code
# was to confirm that we didn't miss any
if len(files) != 489:
...
Groups = [
    ...
    56,
    57,
    59]  # Groups 44 and 50 were eliminated due to shorter time series length(<2min)
...
for grp in Groups:
    # determining the number of members in the group
    d = int(len(extracted_data['preGD_THS'][grp]))
    if grp == 30:
        data = np.zeros((115, d))
        data_rn = np.zeros((150, d))
        for member in range(d):
            # we need to do this because python indexing starts from zero
            # whereas the assigned numbers of group members started from 1
            array = extracted_data['preGD_THS'][grp][member + 1]
            data[:, member] = array[10:125]
            data_rn[:, member] = np.random.permutation(
                array[10:125])  # randomized data

        np.save('/home/swarag0/Group_study/data/(preGD,' +
                str(grp) +
                ',' +
                str(outcome[GroupID == grp][0]) +
                ').npy', data)  # save data with name (type of data, group, outcome)
        np.save('/home/swarag0/Group_study/data/(rn_preGD,' + str(grp) +
                ',' + str(outcome[GroupID == grp][0]) + ').npy', data_rn)
...
for grp in Groups:
    # determining the number of members in the group
    d = int(len(extracted_data['GD_THS'][grp]))
    data = np.zeros((115, d))
    data_rn = np.zeros((150, d))
    for member in range(d):
        # we need to do this because python indexing starts from zero whereas
        # the assigned numbers of group members started from 1
        array = extracted_data['GD_THS'][grp][member + 1]
        data[:, member] = array
        data_rn[:, member] = np.random.permutation(array)  # randomized data

    np.save('/home/swarag0/Group_study/data/(GD,' +
            str(grp) +
            ',' +
            str(outcome[GroupID == grp][0]) +
            ').npy', data)  # save data with name (type of data, group, outcome)
    np.save('/home/swarag0/Group_study/data/(rn_GD,' + str(grp) +
            ',' + str(outcome[GroupID == grp][0]) + ').npy', data_rn)
...
# a perception that the entire analysis could be done by running a single
# script.
...
Dict_RPs = windowed_RP(
    68,
    '/home/swarag0/Group_study/RP',
    '/home/swarag0/Group_study')

First_middle_last_sliding_windows_all_vars(
    Dict_RPs, '/home/swarag0/Group_study/Group_RQA_data.csv')
...
# example comparison:  Between GR and random GD, and selecting mode of
...
rn_GD_data = rn_GD_data_1[rn_GD_data_1['window']
                          == 'mode'].reset_index(drop=True)

# IMPORTANT: Both the data should have the same ordering if we are
# planning to use pair-wise comparison for outerloop validation accuracies
