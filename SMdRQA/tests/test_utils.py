...
class TestCompute3DMatrixSize(unittest.TestCase):
    ...
    def test_random_float64_matrix(self):
        ...
        # Assert the two sizes are almost equal (due to floating-point precision)
...
class TestAssert3DMatrixSize(unittest.TestCase):
    ...
    def test_memory_within_limit(self):
        # Test case where the memory requirement should be within the specified limit
        dim1, dim2, dim3 = 100, 100, 100  # Adjust dimensions to be within the 4 GiB limit
        memory_limit = 4  # GiB
        self.assertTrue(assert_3D_matrix_size(dim1, dim2, dim3, dtype=np.float64, memory_limit=memory_limit))

    def test_memory_exceeds_limit(self):
        # Test case where the memory requirement should exceed the specified limit
        dim1, dim2, dim3 = 1000, 1000, 1000  # Dimensions likely to exceed 4 GiB
        memory_limit = 4  # GiB
        self.assertFalse(assert_3D_matrix_size(dim1, dim2, dim3, dtype=np.float64, memory_limit=memory_limit))

    def test_different_dtype_within_limit(self):
        # Test case with a different data type, e.g., float32, which uses less memory
        dim1, dim2, dim3 = 500, 500, 500
        memory_limit = 2  # GiB
        self.assertTrue(assert_3D_matrix_size(dim1, dim2, dim3, dtype=np.float32, memory_limit=memory_limit))
    ...
    def test_large_memory_limit(self):
        # Test case with a large memory limit where any reasonable matrix size would pass
