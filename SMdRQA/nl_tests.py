...
def surrogate(sig, N, method, pp, fs, *args):
    ...
    * Theiler, James. "On the evidence for low-dimensional chaos in an epileptic electroencephalogram." Physics Letters A 196.1-2 (1994): 335-341.
    ...
    # Amplitude adjusted Fourier transform surrogate
        ...
        gn = sort_matlab(np.random.randn(N, len(sig)), dim=1) # Create Gaussian noise signal and np.np.sort
        for j in range(N):
    ...
    # Pseudo-periodic surrogates (PPS)
        ...
        for x in range(N):
            ...
            for j in range(L): # Length of surrogate is the same as the embedded time series
                if kn == L:
                    kn = pl
                kn += 1 # Move forward from previous kn
                surr[x, j] = sig[0, kn] # Set surrogate to current value for kn (choose first component, can be any)
                sigdist = max(abs(sig[:, :]-sig[:, kn]*ones(1, L))) # Find the maximum 
    ...
    # Twin surrogates
        ...
        while len(remp) > 0:
            twn = remp[0]
            ind[twn] = remp[max(abs(Rij[:, remp]-Rij[:, twn]*ones(1, len(remp)))) == 0]
            ind[ind[twn]] = ind[twn]
